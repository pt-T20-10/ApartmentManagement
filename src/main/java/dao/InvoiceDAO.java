...
public class InvoiceDAO {
    ...
    public List<Invoice> getInvoicesByMonth(int month, int year) {
        ...
        try (Connection conn = Db_connection.getConnection(); PreparedStatement ps = conn.prepareStatement(sql)) {
            ...
            while (rs.next()) invoices.add(mapRowToInvoice(rs));
        } catch (SQLException e) { e.printStackTrace(); }
        return invoices;
    }
    ...
    public BigDecimal getTotalRevenueByBuilding(Long buildingId) {
        ...
        try (Connection conn = Db_connection.getConnection(); PreparedStatement ps = conn.prepareStatement(sql)) {
            ...
            try (ResultSet rs = ps.executeQuery()) { if (rs.next()) return rs.getBigDecimal(1) != null ? rs.getBigDecimal(1) : BigDecimal.ZERO; }
        } catch (SQLException e) { e.printStackTrace(); }
        return BigDecimal.ZERO;
    }
