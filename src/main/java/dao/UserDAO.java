 ...
 * UPDATED: Soft Delete Logic (is_deleted)
 */
public class UserDAO {
    ...
    public List<User> getUsersByBuilding(Long buildingId) {
        ...
        String sql = "SELECT u.*, b.name AS building_name, " +
                     ...
                     "LEFT JOIN buildings b ON u.building_id = b.id " +
                     "LEFT JOIN users a ON u.assigned_by = a.id " +
                     "WHERE u.building_id = ? AND u.is_deleted = 0 " + // ✅ Check deleted
    ...
    public boolean insertUser(User user, User createdBy) {
        ...
        try (Connection conn = Db_connection.getConnection();
            ...
            if (user.getBuildingId() != null) pstmt.setLong(6, user.getBuildingId());
            else pstmt.setNull(6, Types.BIGINT);
            
            if (createdBy != null) pstmt.setLong(7, createdBy.getId());
    ...
    public boolean updateUser(User user) {
        ...
        try (Connection conn = Db_connection.getConnection();
            ...
            if (user.getBuildingId() != null) pstmt.setLong(5, user.getBuildingId());
    ...
    public boolean assignUserToBuilding(Long userId, Long buildingId, User assignedBy) {
        ...
        try (Connection conn = Db_connection.getConnection();
            ...
            if (buildingId != null) ps.setLong(1, buildingId);
            else ps.setNull(1, Types.BIGINT);
            
            if (assignedBy != null) ps.setLong(2, assignedBy.getId());
    ...
    public List<User> getAllActiveUsers() {
        ...
        String sql = "SELECT u.*, b.name AS building_name, " +
                     ...
                     "LEFT JOIN buildings b ON u.building_id = b.id " +
                     "LEFT JOIN users a ON u.assigned_by = a.id " +
                     "WHERE u.is_active = 1 AND u.is_deleted = 0 " + // ✅ Check deleted
     ...
     * ✅ Đã sửa: Chuyển is_deleted = 1 và is_active = 0
     */
    ...
    private User mapResultSetToUser(ResultSet rs) throws SQLException {
        ...
        try { user.setBuildingName(rs.getString("building_name")); } catch (SQLException e) { user.setBuildingName(null); }
    ...
    public List<User> getAllManagers() {
        ...
        try (Connection conn = Db_connection.getConnection();
             Statement stmt = conn.createStatement();
             ResultSet rs = stmt.executeQuery(sql)) {
            while (rs.next()) managers.add(mapResultSetToUser(rs));
        } catch (SQLException e) {
            e.printStackTrace();
        }
        return managers;
    ...
    public List<User> getManagersForBuilding(Long currentManagerId) {
        ...
        try (Connection conn = Db_connection.getConnection();
             PreparedStatement ps = conn.prepareStatement(sql)) {
            if (currentManagerId != null) ps.setLong(1, currentManagerId);
            else ps.setNull(1, Types.BIGINT);
            ResultSet rs = ps.executeQuery();
            while (rs.next()) managers.add(mapResultSetToUser(rs));
        } catch (SQLException e) {
