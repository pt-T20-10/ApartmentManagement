 ...
 * - RENTAL: Has start_date and end_date
 * - OWNERSHIP: NO start_date and end_date (only signed_date)
 */
public class ContractDAO {
    ...
    public boolean insertContract(Contract contract) {
        String sqlContract = "INSERT INTO contracts " +
                           ...
                           "signed_date, start_date, end_date, deposit_amount, monthly_rent, " +
        ...
        try (Connection conn = Db_connection.getConnection()) {
            ...
            try {
                ...
                try (PreparedStatement pstC = conn.prepareStatement(sqlContract, Statement.RETURN_GENERATED_KEYS)) {
                    int idx = 1;
                    pstC.setString(idx++, contract.getContractNumber());
                    pstC.setLong(idx++, contract.getApartmentId());
                    pstC.setLong(idx++, contract.getResidentId());
                    pstC.setString(idx++, contract.getContractType());
                    
                    // ✅ signed_date (nullable)
                    if (contract.getSignedDate() != null) {
                        pstC.setDate(idx++, new java.sql.Date(contract.getSignedDate().getTime()));
                    } else {
                        pstC.setNull(idx++, Types.DATE);
                    }
                    
                    // ✅ start_date (NULL for OWNERSHIP)
                    if (contract.getStartDate() != null) {
                        pstC.setDate(idx++, new java.sql.Date(contract.getStartDate().getTime()));
                    } else {
                        pstC.setNull(idx++, Types.DATE);
                    }
                    ...
                    if (contract.getEndDate() != null) {
                        pstC.setDate(idx++, new java.sql.Date(contract.getEndDate().getTime()));
                    } else {
                        pstC.setNull(idx++, Types.DATE);
                    }
                    
                    pstC.setBigDecimal(idx++, contract.getDepositAmount());
                    pstC.setBigDecimal(idx++, contract.getMonthlyRent()); // ✅ Dual purpose
                    pstC.setString(idx++, contract.getStatus());
                    pstC.setString(idx++, contract.getNotes());
                ...
                if (generatedId != null) {
                    ...
                    history.setReason("Tạo hợp đồng " + contract.getContractTypeDisplay());
                    history.setCreatedBy(getCurrentUserId());
                    contractHistoryDAO.insert(conn, history);
                }

                conn.commit();
                System.out.println("✅ Contract inserted successfully: " + contract.getContractNumber());
                return true;
                
            } catch (SQLException e) {
                conn.rollback();
                System.err.println("❌ Transaction rolled back in insertContract()");
                throw e;
            }
    ...
    public List<Contract> getExpiringContracts(int daysThreshold) {
        List<Contract> contracts = new ArrayList<>();
        String sql = "SELECT * FROM contracts " +
                     "WHERE contract_type = 'RENTAL' " + // ✅ Only RENTAL contracts expire
