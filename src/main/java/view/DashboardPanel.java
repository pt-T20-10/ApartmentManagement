...
public class DashboardPanel extends JPanel {
    ...
    private int getApartmentCount() { return apartmentDAO.countApartmentsByBuilding(currentUser.getBuildingId()); }
    private int getAvailableApartmentCount() { return apartmentDAO.countAvailableApartmentsByBuilding(currentUser.getBuildingId()); }
    private int getRentedApartmentCount() { return apartmentDAO.countRentedApartmentsByBuilding(currentUser.getBuildingId()); }
    private int getResidentCount() { return residentDAO.countResidentsByBuilding(currentUser.getBuildingId()); }
    private BigDecimal getTotalRevenue() { return invoiceDAO.getTotalRevenueByBuilding(currentUser.getBuildingId()); }
    private int getUnpaidInvoicesCount() { return invoiceDAO.countUnpaidInvoicesByBuilding(currentUser.getBuildingId()); }
    ...
    private JPanel createKPISection() {
        ...
        section.add(row1); section.add(Box.createVerticalStrut(15));
        section.add(row2); section.add(Box.createVerticalStrut(15));
        section.add(row3);
        return section;
    }
    ...
    private JPanel createStatCard(String title, String value, String icon, Color color, Runnable onClick) {
        ...
        textPanel.add(lblTitle); textPanel.add(lblValue);
        card.add(textPanel, BorderLayout.CENTER);
        
        if(onClick != null) card.addMouseListener(new MouseAdapter() { public void mouseClicked(MouseEvent e) { onClick.run(); } });
        return card;
    }
    ...
    private JPanel createRevenueCard() {
        ...
        JPanel p = new JPanel(); p.setLayout(new BoxLayout(p, BoxLayout.Y_AXIS)); p.setBackground(CARD_BG);
    ...
    private class PieChartPanel extends JPanel {
        ...
        @Override protected void paintComponent(Graphics g) {
            ...
            // Legend
            g2.setColor(c1); g2.fillRect(w-100, h-40, 10, 10);
            g2.setColor(Color.BLACK); g2.drawString("Đã thu (" + paid + ")", w-85, h-30);
            
            g2.setColor(c2); g2.fillRect(w-100, h-20, 10, 10);
            g2.setColor(Color.BLACK); g2.drawString("Chưa thu (" + unpaid + ")", w-85, h-10);
        }
    }
}