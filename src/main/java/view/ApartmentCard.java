...
public class ApartmentCard extends JPanel {
    ...
    public ApartmentCard(Apartment apartment, LocalDate contractEndDate,
        ...
        this.addMouseListener(new MouseAdapter() {
            ...
            public void mouseClicked(MouseEvent e) {
                if (onSelect != null) onSelect.accept(apartment);
            }
            @Override
    ...
    private void initUI() {
        ...
        // Bên trái: Thông tin/Cảnh báo
        JComponent leftInfo = createFooterInfo();
        footerPanel.add(leftInfo, BorderLayout.WEST);
    ...
    private JComponent createFooterInfo() {
        if (contractEndDate != null && ("RENTED".equalsIgnoreCase(apartment.getStatus()) || "Đã thuê".equalsIgnoreCase(apartment.getStatus()))) {
            ...
            if (days < 0) {
                lblAlert.setText(" ĐÃ QUÁ HẠN " + Math.abs(days) + " NGÀY");
                lblAlert.setForeground(new Color(211, 47, 47));
                lblAlert.setIcon(new CardIcon("WARNING", 14, new Color(211, 47, 47))); // Icon Cảnh báo
                return lblAlert;
            } else if (days <= 30) {
                lblAlert.setText(" Hết hạn: " + days + " ngày");
                lblAlert.setForeground(new Color(230, 81, 0));
                lblAlert.setIcon(new CardIcon("TIME", 14, new Color(230, 81, 0))); // Icon Đồng hồ
                return lblAlert;
            }
        }
    ...
    protected void paintComponent(Graphics g) {
        ...
        if (contractEndDate != null && ChronoUnit.DAYS.between(LocalDate.now(), contractEndDate) < 0 
            && ("RENTED".equalsIgnoreCase(apartment.getStatus()) || "Đã thuê".equalsIgnoreCase(apartment.getStatus()))) {
    ...
    private StatusBadge createStatusBadge(String status) {
        if ("RENTED".equalsIgnoreCase(status) || "Đã thuê".equalsIgnoreCase(status)) return new StatusBadge("Đã thuê", new Color(232, 245, 233), new Color(46, 125, 50));
        if ("MAINTENANCE".equalsIgnoreCase(status) || "Bảo trì".equalsIgnoreCase(status)) return new StatusBadge("Bảo trì", new Color(255, 243, 224), new Color(239, 108, 0));
    ...
    // --- [QUAN TRỌNG] CLASS VẼ ICON (ĐÃ CÓ WARNING VÀ TIME) ---
    private static class CardIcon implements Icon { 
        ...
        @Override public void paintIcon(Component c, Graphics g, int x, int y) { 
            ...
            if ("EDIT".equals(type)) { 
                ...
                Path2D tip = new Path2D.Float(); tip.moveTo(size/2-2, size-4); tip.lineTo(size/2, size); tip.lineTo(size/2+2, size-4); g2.fill(tip); 
            } 
            ...
            // [VẼ ICON CẢNH BÁO TAM GIÁC]
            else if ("WARNING".equals(type)) { 
                Path2D p = new Path2D.Float(); 
                p.moveTo(size/2.0, 1); // Đỉnh
                p.lineTo(1, size-1);   // Trái dưới
                p.lineTo(size-1, size-1); // Phải dưới
                p.closePath(); 
                g2.draw(p); 
                // Dấu chấm than
            ...
            // [VẼ ICON ĐỒNG HỒ]
                ...
                g2.drawOval(1, 1, size-2, size-2); // Mặt đồng hồ
                g2.drawLine(size/2, 4, size/2, size/2); // Kim giờ
                g2.drawLine(size/2, size/2, size-4, size/2); // Kim phút
            }
