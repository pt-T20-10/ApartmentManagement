...
public class ApartmentManagementPanel extends JPanel {
    ...
    private List<Apartment> filterByStatus(List<Apartment> apartments, String statusFilter) {
        ...
        for (Apartment apt : apartments) {
            String aptStatus = apt.getStatus(); if (aptStatus == null) aptStatus = "AVAILABLE"; boolean match = false;
            switch (statusFilter) { case "Trống": match = "AVAILABLE".equalsIgnoreCase(aptStatus); break; case "Đã thuê": match = "RENTED".equalsIgnoreCase(aptStatus); break; case "Đã bán": match = "OWNED".equalsIgnoreCase(aptStatus); break; case "Bảo trì": match = "MAINTENANCE".equalsIgnoreCase(aptStatus); break; default: match = true; }
            if (match) filtered.add(apt);
        } return filtered;
    }
    private void showAddDialog() { if (currentBuilding == null) { JOptionPane.showMessageDialog(this, "Vui lòng chọn tòa nhà trước!", "Thông báo", JOptionPane.WARNING_MESSAGE); return; } JFrame parent = (JFrame) SwingUtilities.getWindowAncestor(this); Apartment newApt = new Apartment(); if (currentFloor != null) newApt.setFloorId(currentFloor.getId()); ApartmentDialog dialog = new ApartmentDialog(parent, newApt, currentBuilding.getId()); dialog.setVisible(true); if (dialog.isConfirmed()) { Apartment apt = dialog.getApartment(); if (isRoomNumberExists(apt.getFloorId(), apt.getRoomNumber(), null)) { JOptionPane.showMessageDialog(this, "Số phòng " + apt.getRoomNumber() + " đã tồn tại!", "Lỗi", JOptionPane.ERROR_MESSAGE); return; } if (apartmentDAO.insertApartment(apt)) { JOptionPane.showMessageDialog(this, "Thêm thành công!"); loadApartments(); } else { JOptionPane.showMessageDialog(this, "Thêm thất bại!"); } } }
    private void editApartment(Apartment apt) { JFrame parent = (JFrame) SwingUtilities.getWindowAncestor(this); ApartmentDialog dialog = new ApartmentDialog(parent, apt, currentBuilding.getId()); dialog.setVisible(true); if (dialog.isConfirmed()) { Apartment updated = dialog.getApartment(); if (isRoomNumberExists(updated.getFloorId(), updated.getRoomNumber(), apt.getId())) { JOptionPane.showMessageDialog(this, "Số phòng " + updated.getRoomNumber() + " đã tồn tại!", "Lỗi", JOptionPane.ERROR_MESSAGE); return; } if (apartmentDAO.updateApartment(updated)) { JOptionPane.showMessageDialog(this, "Cập nhật thành công!"); loadApartments(); } else { JOptionPane.showMessageDialog(this, "Cập nhật thất bại!"); } } }
    private void deleteApartment(Apartment apt) { String status = (apt.getStatus() == null) ? "" : apt.getStatus(); if (status.equalsIgnoreCase("RENTED")) { JOptionPane.showMessageDialog(this, "KHÔNG THỂ XÓA CĂN HỘ NÀY!\n\n" + "Lý do: Căn hộ " + apt.getRoomNumber() + " đang có người thuê (Hợp đồng RENTAL).\n" + "Để đảm bảo an toàn dữ liệu, vui lòng thanh lý hợp đồng trước khi xóa.", "Thao tác bị chặn", JOptionPane.ERROR_MESSAGE); return; } if (status.equalsIgnoreCase("OWNED")) { JOptionPane.showMessageDialog(this, "KHÔNG THỂ XÓA CĂN HỘ NÀY!\n\n" + "Lý do: Căn hộ " + apt.getRoomNumber() + " đã được bán (Hợp đồng OWNERSHIP).\n" + "Căn hộ đã bán không thể xóa khỏi hệ thống.", "Thao tác bị chặn", JOptionPane.ERROR_MESSAGE); return; } int confirm = JOptionPane.showConfirmDialog(this, "Bạn có chắc chắn muốn xóa căn hộ " + apt.getRoomNumber() + "?\n" + "Dữ liệu sẽ được chuyển vào thùng rác (Xóa mềm).", "Xác nhận xóa", JOptionPane.YES_NO_OPTION); if (confirm == JOptionPane.YES_OPTION) { if (apartmentDAO.deleteApartment(apt.getId())) { JOptionPane.showMessageDialog(this, "Đã xóa thành công!"); loadApartments(); } else { JOptionPane.showMessageDialog(this, "Xóa thất bại!", "Lỗi", JOptionPane.ERROR_MESSAGE); } } }
    private boolean isRoomNumberExists(Long floorId, String roomNumber, Long excludeApartmentId) { if (floorId == null || roomNumber == null) return false; List<Apartment> apartments = apartmentDAO.getApartmentsByFloorId(floorId); for (Apartment apt : apartments) { if (excludeApartmentId != null && apt.getId().equals(excludeApartmentId)) continue; if (roomNumber.trim().equalsIgnoreCase(apt.getRoomNumber().trim())) return true; } return false; }
    private JButton createBackArrowButton() { JButton btn = new JButton("Quay lại"); btn.setIcon(new HeaderIcon("BACK_ARROW", 16, UIConstants.PRIMARY_COLOR)); btn.setFont(new Font("Segoe UI", Font.BOLD, 14)); btn.setForeground(UIConstants.PRIMARY_COLOR); btn.setBackground(Color.WHITE); btn.setContentAreaFilled(false); btn.setBorder(new EmptyBorder(5, 10, 5, 15)); btn.setFocusPainted(false); btn.setCursor(new Cursor(Cursor.HAND_CURSOR)); btn.addMouseListener(new MouseAdapter() { public void mouseEntered(MouseEvent e) { btn.setContentAreaFilled(true); btn.setBackground(new Color(235, 245, 255)); } public void mouseExited(MouseEvent e) { btn.setContentAreaFilled(false); } }); return btn; }
    private static class RoundedButton extends JButton { private int arc; public RoundedButton(String text, int arc) { super(text); this.arc = arc; setContentAreaFilled(false); setBorderPainted(false); setFocusPainted(false); setCursor(new Cursor(Cursor.HAND_CURSOR)); } @Override protected void paintComponent(Graphics g) { Graphics2D g2 = (Graphics2D) g.create(); g2.setRenderingHint(RenderingHints.KEY_ANTIALIASING, RenderingHints.VALUE_ANTIALIAS_ON); g2.setColor(getBackground()); g2.fillRoundRect(0, 0, getWidth(), getHeight(), arc, arc); super.paintComponent(g); g2.dispose(); } }
    private static class HeaderIcon implements Icon { private String type; private int size; private Color color; public HeaderIcon(String type, int size, Color color) { this.type = type; this.size = size; this.color = color; } @Override public void paintIcon(Component c, Graphics g, int x, int y) { Graphics2D g2 = (Graphics2D) g.create(); g2.setRenderingHint(RenderingHints.KEY_ANTIALIASING, RenderingHints.VALUE_ANTIALIAS_ON); g2.setColor(color); g2.setStroke(new BasicStroke(2.0f, BasicStroke.CAP_ROUND, BasicStroke.JOIN_ROUND)); g2.translate(x, y); if ("BACK_ARROW".equals(type)) { Path2D p = new Path2D.Float(); p.moveTo(size * 0.7, size * 0.2); p.lineTo(size * 0.3, size * 0.5); p.lineTo(size * 0.7, size * 0.8); g2.draw(p); } g2.dispose(); } @Override public int getIconWidth() { return size; } @Override public int getIconHeight() { return size; } }
}