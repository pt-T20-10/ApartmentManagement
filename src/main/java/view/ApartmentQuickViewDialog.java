...
public class ApartmentQuickViewDialog extends JDialog {
    ...
    private void loadRealData() {
        if (apartment.getId() == null) return;
    ...
    private JLabel createStatusBadge(String status) {
        ...
        if ("RENTED".equalsIgnoreCase(s) || "Đã thuê".equalsIgnoreCase(s) || "OCCUPIED".equalsIgnoreCase(s)) { 
            bg = new Color(232, 245, 233); fg = new Color(46, 125, 50); text = "ĐANG THUÊ"; 
    ...
    private static class RoundedButton extends JButton { 
        private int arc; 
        public RoundedButton(String text, int arc) { super(text); this.arc = arc; setContentAreaFilled(false); setFocusPainted(false); setBorderPainted(false); setCursor(new Cursor(Cursor.HAND_CURSOR)); setFont(new Font("Segoe UI", Font.BOLD, 12)); } 
    ...
    private static class StatusDotIcon implements Icon { 
        ...
        @Override public void paintIcon(Component c, Graphics g, int x, int y) { Graphics2D g2 = (Graphics2D) g.create(); g2.setRenderingHint(RenderingHints.KEY_ANTIALIASING, RenderingHints.VALUE_ANTIALIAS_ON); g2.setColor(color); g2.fillOval(x + 2, y + 4, 8, 8); g2.dispose(); } 
