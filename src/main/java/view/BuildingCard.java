...
public class BuildingCard extends JPanel {
    ...
    private void initUI() {
        ...
        // === CENTER & BOTTOM ===
        JPanel centerPanel = new JPanel(new BorderLayout()); centerPanel.setOpaque(false); centerPanel.setBorder(new EmptyBorder(15, 0, 0, 0));
        ...
        content.add(topPanel, BorderLayout.NORTH); content.add(centerPanel, BorderLayout.CENTER); content.add(bottomPanel, BorderLayout.SOUTH);
    ...
    private static class StatusBadge extends JLabel {
        ...
        public StatusBadge(String rawStatus) {
            ...
            if (normalized.equalsIgnoreCase("ACTIVE") || normalized.equalsIgnoreCase("Hoạt động") || normalized.equalsIgnoreCase("Đang hoạt động")) {
                displayText = "Đang hoạt động"; bgColor = new Color(232, 245, 233); textColor = new Color(46, 125, 50);
            } else if (normalized.equalsIgnoreCase("MAINTENANCE") || normalized.toLowerCase().contains("bảo trì")) {
                displayText = "Đang bảo trì"; bgColor = new Color(255, 243, 224); textColor = new Color(239, 108, 0);
            } else {
                bgColor = new Color(245, 245, 245); textColor = new Color(97, 97, 97);
            }
            setText(displayText); setFont(new Font("Segoe UI", Font.BOLD, 12)); setBorder(new EmptyBorder(5, 12, 5, 12)); setForeground(textColor);
        }
        @Override protected void paintComponent(Graphics g) {
            Graphics2D g2 = (Graphics2D) g.create(); g2.setRenderingHint(RenderingHints.KEY_ANTIALIASING, RenderingHints.VALUE_ANTIALIAS_ON);
    ...
    private JLabel createStatLabel(String iconType, String text) { JLabel l = new JLabel(" " + text); l.setIcon(new SimpleIcon(iconType, 14, Color.GRAY)); l.setFont(new Font("Segoe UI", Font.PLAIN, 13)); l.setForeground(Color.GRAY); return l; }
    private JButton createIconButton(String iconType, Color color) { JButton btn = new JButton(new SimpleIcon(iconType, 22, color)); btn.setBorderPainted(false); btn.setContentAreaFilled(false); btn.setFocusPainted(false); btn.setCursor(new Cursor(Cursor.HAND_CURSOR)); return btn; }
